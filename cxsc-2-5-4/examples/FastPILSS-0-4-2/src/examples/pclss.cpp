...
/*
** Example program for complex point systems. 
**
...
//Function defining elements of A
void makeA(int i, int j, complex& r) {
  r = sqrt(2.0/(n+1))*sin((i*j*M_PI)/(n+1)); 
  r *= complex(1,-1);
}

//Function defining elements of b
void makeb(int i, int j, complex& r) {
...
static real RelErr(const civector &x) {
  return (RelErr(Re(x)) + RelErr(Im(x))) / 2;
}
...
int main (int argc, char** argv)
{
  ...
  A = randn(m,n) + randn(m,n) * complex(0,1);
  ...
  pclss(A,b,x,m,n,np,myid,Err,out,cfg);
  //Version 2 using function pointers to define A and b (allowing larger systems)
  //pclss(makeA,makeb,x,m,n,rhs,np,myid,Err,out,cfg);
  end = GetTime();
