...
//      friend inline interval _interval(const idotprecision &a) throw() { return interval(a); }
      ...
      // ---- Destruktor    ----
      // ~interval() { delete [] inf; delete [] sup; }; // unnoetig - AT editted; original = commented
...
//             inline      operator void*(void) const throw() { return (void*)(!(!*this)); } 
      ...
      // returns true if the interval is proper
      friend inline bool IsImproper(const interval &) throw();
      friend inline bool IsProper(const interval &) throw();

      //! Returns the absolute value of the interval
      friend inline interval abs  (const interval &) throw();
      //! Returns the exponential value of the interval. AT edit - 2Dec15
      friend inline interval exponential  (const interval &) throw();
      //! Returns the dual value of the interval. AT edit - 19Dec15
      friend inline interval dual (const interval &) throw(); 
      // AT:add rad - return proper projection of interval
      friend inline interval prop (const interval &) throw(); 
      ...
      // AT:add rad - return the radius of the interval
      friend inline real     radi (const interval &) throw();
