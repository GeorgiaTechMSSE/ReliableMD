...
#undef LINT_ARGS
#define CXSC_INCLUDE
#include "fi_lib.hpp" // AT: change quotation mark; previous: #include <fi_lib.hpp>
...
// AT: add pow(interval, real) for LAMMPS LJ implementation
interval pow    (const interval &a, const real &b) throw() 
       {
          // only handle int b case;
          // double b_double = _double(b);
          return interval( pow(_double(a.inf),_double(b)) , pow(_double(a.sup),_double(b)) );
       }
