...
#include "real.hpp"
// Added by AT. 19Dec15
// #include "imath.hpp" // Editted by AT: include exp() function but ambiguity for real& and interval&
#include "fi_lib.hpp" // Editted by AT
// #include "imath.inl"
// #include "imatrix.hpp"
// #include "ivector.hpp"
...
// #include <iostream>
#include <cmath> // add exponential

#include <math.h> // for Lakeyev equivalent formula

namespace cxsc {

// ---- Konstruktoren ----

// Editted by AT. 19Dec15 -- comment: need inf and sup
inline interval::interval(const real &a, const real &b): inf(a), sup(b) {}
...
// inline interval operator-(const interval &a) throw() { return interval (-a.sup, -a.inf); } // classical interval
inline interval operator-(const interval &a) throw() { return interval (-a.inf, -a.sup); } // Kaucher   interval
...
inline bool IsImproper(const interval &a) throw() { return (a.inf>a.sup); }
inline bool IsProper(const interval &a) throw() { return (a.sup>a.inf); } 
...
// Added by AT - 2Dec15
inline interval exponential(const interval& a) throw() 
{
  // interval h = fi_lib::j_exp(a);
  // real h1 = h.inf;
  // real h2 = h.sup;

  double a1 = _double(a.inf);
  double a2 = _double(a.sup);
  // std::cout << "h.inf = " << h.inf << std::endl;
  // std::cout << "h.sup = " << h.sup << std::endl;
  // std::cout << "a.inf = " << a.inf << std::endl;
  // std::cout << "a.sup = " << a.sup << std::endl;
  return interval(std::exp(a1),std::exp(a2));
}

// Added by AT - 19Dec15
inline interval dual(const interval& a) throw()
{
  return interval(a.sup,a.inf);
}

// Added by AT - 26May16
inline interval prop(const interval& a) throw()
{
  return interval( min(a.sup,a.inf),max(a.sup,a.inf) );
}
...
// AT:add rad
inline real radi(const interval & a) throw()
{
   return diam(a)/2;
}
...
/*  ----------------------- KAUCHER (DUAL OPERATOR) ----------------------- */

  interval operator-  (const interval& a, const interval& b) throw() 
  { return interval ( subdown(a.inf,b.inf), subup(a.sup,b.sup)); }
  interval operator-  (const interval& a, const real& b) throw() 
  { return interval ( subdown(a.inf,b), subup(a.sup,b)); }
  interval operator-  (const real& a, const interval& b) throw() 
  { return interval ( subdown(a,b.inf), subup(a,b.sup)); }
...
// KAUCHER interval implementation -- could be replaced by Lakeyev formulation
interval operator *(const interval& a, const interval& b) throw()
{
   interval tmp;

  /* ------------------------------------------------------------------
    depreciate multup and multdown due to accumulation of accuracy lost 
  ------------------------------------------------------------------ */
  /*

  // Kaucher multiplication table: deappreciated

  // a in P subspace
  // a in P, b in P
  if (0 <= a.inf && a.sup >= 0 && 0 <= b.inf && 0 <= b.sup)
    {tmp.inf = multdown(a.inf,b.inf);
        tmp.sup = multup(a.sup,b.sup);}
  // a in P, b in Z
  if (0 <= a.inf && a.sup >= 0 && b.inf <= 0 && 0 <= b.sup)
    {tmp.inf = multdown(a.sup,b.inf);
        tmp.sup = multup(a.sup,b.sup);}
  // a in P, b in -P
  if (0 <= a.inf && a.sup >= 0 && b.inf <= 0 && b.sup <= 0)
    {tmp.inf = multdown(a.sup,b.inf);
        tmp.sup = multup(a.inf,b.sup);}
  // a in P, b in dual Z
  if (0 <= a.inf && a.sup >= 0 && 0 <= b.inf && b.sup <= 0)
    {tmp.inf = multdown(a.inf,b.inf);
        tmp.sup = multup(a.inf,b.sup);}

  // a in Z subspace
  // a in Z, b in P
  if (a.inf <= 0 && a.sup >= 0 && 0 <= b.inf && 0 <= b.sup)
    {tmp.inf = multdown(a.inf,b.sup);
        tmp.sup = multup(a.sup,b.sup);}
  // a in Z, b in Z
  if (a.inf <= 0 && a.sup >= 0 && b.inf <= 0 && 0 <= b.sup)
    {tmp.inf = min(multdown(a.inf,b.sup),multdown(a.sup,b.inf));
        tmp.sup = max(multup(a.inf,b.inf),multup(a.sup,b.sup));}
  // a in Z, b in -P
  if (a.inf <= 0 && a.sup >= 0 && b.inf <= 0 && b.sup <= 0)
    {tmp.inf = multdown(a.sup,b.inf);
        tmp.sup = multup(a.inf,b.inf);}
  // a in Z, b in dual Z
  if (a.inf <= 0 && a.sup >= 0 && 0 <= b.inf && b.sup <= 0)
    {tmp.inf = 0.0;
        tmp.sup = 0.0;}
  ...
  if (0 <= a.inf && a.sup <= 0 && b.inf <= 0 && b.sup <= 0)
    {tmp.inf = multdown(a.sup,b.sup);
        tmp.sup = multup(a.inf,b.sup);}
  // a in dual Z, b in dual Z
  if (0 <= a.inf && a.sup <= 0 && 0 <= b.inf && b.sup <= 0)
    {tmp.inf = max(multdown(a.inf,b.inf),multdown(a.sup,b.sup));
        tmp.sup = min(multup(a.inf,b.sup),multup(a.sup,b.inf));}

  */

  /* ------------------------------------------------------------------
    depreciate Lakeyev formulation due to accumulation of accuracy lost; depreciate to improve computational efficiency
  ------------------------------------------------------------------ */
  /*// Lakeyev equivalent formula:
  
  double arad = _double(0.5*(a.sup - a.inf));
  double brad = _double(0.5*(b.sup - b.inf));
  double amid = _double(0.5*(a.inf + a.sup));
  double bmid = _double(0.5*(b.inf + b.sup));

  tmp.inf = fmax(fmax(amid-arad,0) * fmax(bmid-brad,0) , fmax(-amid-arad,0) * fmax(-bmid-brad,0))-fmax(fmax(amid+arad,0) * fmax(brad-bmid,0) , fmax(arad-amid,0) * fmax(bmid+brad,0));

  tmp.sup = fmax(fmax(amid+arad,0) * fmax(bmid+brad,0) , fmax(-amid+arad,0) * fmax(-bmid+brad,0))-fmax(fmax(amid-arad,0) * fmax(-bmid-brad,0) , fmax(-amid-arad,0) * fmax(bmid-brad,0));*/
  

  /* ------------------------------------------------------------------
    update Kaucher table with reverse _double to enhance accuracy; depreciate to improve computational efficiency
  ------------------------------------------------------------------ */

  /* // a in P subspace
  // a in P, b in P
  if (0 <= a.inf && a.sup >= 0 && 0 <= b.inf && 0 <= b.sup)
    {tmp.inf = _double(a.inf) * _double(b.inf);
        tmp.sup = _double(a.sup) * _double(b.sup);}
  // a in P, b in Z
  if (0 <= a.inf && a.sup >= 0 && b.inf <= 0 && 0 <= b.sup)
    {tmp.inf = _double(a.sup) * _double(b.inf);
        tmp.sup = _double(a.sup) * _double(b.sup);}
  // a in P, b in -P
  if (0 <= a.inf && a.sup >= 0 && b.inf <= 0 && b.sup <= 0)
    {tmp.inf = _double(a.sup) * _double(b.inf);
        tmp.sup = _double(a.inf) * _double(b.sup);}
  // a in P, b in dual Z
  if (0 <= a.inf && a.sup >= 0 && 0 <= b.inf && b.sup <= 0)
    {tmp.inf = _double(a.inf) * _double(b.inf);
        tmp.sup = _double(a.inf) * _double(b.sup);}
  ...
  // a in Z, b in P
  if (a.inf <= 0 && a.sup >= 0 && 0 <= b.inf && 0 <= b.sup)
    {tmp.inf = _double(a.inf) * _double(b.sup);
        tmp.sup = _double(a.sup) * _double(b.sup);}
  // a in Z, b in Z
  if (a.inf <= 0 && a.sup >= 0 && b.inf <= 0 && 0 <= b.sup)
    {tmp.inf = min( _double(a.inf) * _double(b.sup), _double(a.sup) * _double(b.inf) );
        tmp.sup = max( _double(a.inf) * _double(b.inf), _double(a.sup) * _double(b.sup) );}
  // a in Z, b in -P
  if (a.inf <= 0 && a.sup >= 0 && b.inf <= 0 && b.sup <= 0)
    {tmp.inf = _double(a.sup) * _double(b.inf);
        tmp.sup = _double(a.inf) * _double(b.inf);}
  // a in Z, b in dual Z
  if (a.inf <= 0 && a.sup >= 0 && 0 <= b.inf && b.sup <= 0)
    {tmp.inf = 0.0;
        tmp.sup = 0.0;}
  ...
  if (a.inf <= 0 && a.sup <= 0 && 0 <= b.inf && 0 <= b.sup)
    {tmp.inf = _double(a.inf) * _double(b.sup);
        tmp.sup = _double(a.sup) * _double(b.inf);}
  // a in -P, b in Z
  if (a.inf <= 0 && a.sup <= 0 && b.inf <= 0 && 0 <= b.sup)
    {tmp.inf = _double(a.inf) * _double(b.sup);
        tmp.sup = _double(a.inf) * _double(b.inf);}
  // a in -P, b in -P
  if (a.inf <= 0 && a.sup <= 0 && b.inf <= 0 && b.sup <= 0)
    {tmp.inf = _double(a.sup) * _double(b.sup);
        tmp.sup = _double(a.inf) * _double(b.inf);}
  // a in -P, b in dual Z
  if (a.inf <= 0 && a.sup <= 0 && 0 <= b.inf && b.sup <= 0)
    {tmp.inf = _double(a.sup) * _double(b.sup);
  ...
  // a in dual Z, b in dual Z
  if (0 <= a.inf && a.sup <= 0 && 0 <= b.inf && b.sup <= 0)
    {tmp.inf = max( _double(a.inf) * _double(b.inf), _double(a.sup) * _double(b.sup) );
        tmp.sup = min( _double(a.inf) * _double(b.sup), _double(a.sup) * _double(b.inf) );}
  */

  /* ------------------------------------------------------------------
    Kaucher interval table with if/else syntax to accelerate computation
  ------------------------------------------------------------------ */    
...
interval operator/  (const interval& a, const interval& b) throw(DIV_BY_ZERO)
{
   ...
   if (sign(b.inf*b.sup) <= 0)
      cxscthrow(DIV_BY_ZERO("interval::interval operator/(const interval&,const interval&)"));   
    tmp = interval(a.inf, a.sup)*interval(1/b.sup,1/b.inf);
   return tmp;
}    
