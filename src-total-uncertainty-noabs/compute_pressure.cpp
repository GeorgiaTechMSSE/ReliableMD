...
void ComputePressure::compute_vector()
{
  ...
  if (keflag) {
    ...
    if (atom->xrad_flag && atom->vrad_flag && atom->frad_flag)
      ke_tensor_rad = temperature->vectorrad;
  }
  ...
  if (dimension == 3) {
    ...
    if (keflag) {
      for (int i = 0; i < 6; i++) {
        ...
        // fix here - work!
        if (atom->xrad_flag && atom->vrad_flag && atom->frad_flag)
          vectorrad[i] = (ke_tensor_rad[i] + virialrad[i]) * inv_volume * nktv2p;
      }
    } else
      for (int i = 0; i < 6; i++) {
        ...
        // add rad
        if (atom->xrad_flag && atom->vrad_flag && atom->frad_flag)
          vectorrad[i] = virialrad[i] * inv_volume * nktv2p;
      }
...
void ComputePressure::virial_compute(int n, int ndiag)
{
  ...
  for (j = 0; j < nvirialrad; j++) {
    ...
    for (i = 0; i < n; i++){
      ...
      // std::cout << "vcomponentrad[i] = " << vcomponentrad[i] << std::endl; // fix here - work!
    }
  }
  ...
  // fix here - work!
  if (atom->xrad_flag && atom->vrad_flag && atom->frad_flag)
    MPI_Allreduce(vrad,virialrad,n,MPI_DOUBLE,MPI_SUM,world); // correct version
