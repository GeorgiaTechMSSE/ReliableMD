...
// add interval
// Lakeyev implementation
double rad_DoubleMult(double x, double xrad, double y, double yrad)
{ 
  double inf,sup;
  inf = fmax(fmax(x-xrad,0) * fmax(y-yrad,0) , fmax(-x-xrad,0) * fmax(-y-yrad,0)) 
        - fmax(fmax(x+xrad,0) * fmax(yrad-y,0) , fmax(xrad-x,0) * fmax(y+yrad,0));

  sup = fmax(fmax(x+xrad,0) * fmax(y+yrad,0) , fmax(xrad-x,0) * fmax(yrad-y,0)) 
        - fmax(fmax(x-xrad,0) * fmax(-y-yrad,0) , fmax(-x-xrad,0) * fmax(y-yrad,0));
  return ( 0.5*(sup-inf) );
}

double rad_TripleMult(double x, double xrad, double y, double yrad, double z, double zrad)
{
  double xtimesyinf,xtimesysup;
  xtimesyinf = fmax(fmax(x-xrad,0) * fmax(y-yrad,0) , fmax(-x-xrad,0) * fmax(-y-yrad,0)) 
        - fmax(fmax(x+xrad,0) * fmax(yrad-y,0) , fmax(xrad-x,0) * fmax(y+yrad,0));

  xtimesysup = fmax(fmax(x+xrad,0) * fmax(y+yrad,0) , fmax(xrad-x,0) * fmax(yrad-y,0)) 
        - fmax(fmax(x-xrad,0) * fmax(-y-yrad,0) , fmax(-x-xrad,0) * fmax(y-yrad,0));
  double xtimesy    = 0.5 * ( xtimesyinf + xtimesysup );
  double xtimesyrad = 0.5 * ( xtimesysup - xtimesyinf );

  double inf,sup;  
  inf = fmax(fmax(xtimesy-xtimesyrad,0) * fmax(z-zrad,0) , fmax(-xtimesy-xtimesyrad,0) * fmax(-z-zrad,0)) 
        - fmax(fmax(xtimesy+xtimesyrad,0) * fmax(zrad-z,0) , fmax(xtimesyrad-xtimesy,0) * fmax(z+zrad,0));

  sup = fmax(fmax(xtimesy+xtimesyrad,0) * fmax(z+zrad,0) , fmax(xtimesyrad-xtimesy,0) * fmax(zrad-z,0)) 
        - fmax(fmax(xtimesy-xtimesyrad,0) * fmax(-z-zrad,0) , fmax(-xtimesy-xtimesyrad,0) * fmax(z-zrad,0));
  return ( 0.5 * (sup-inf) );
}
