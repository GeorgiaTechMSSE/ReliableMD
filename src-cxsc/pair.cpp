...
void Pair::ev_setup(int eflag, int vflag)
{
  ...
  // std::cout << "vflag_either = " << vflag_either << std::endl; // = 0 
  // std::cout << "vflag_global = " << vflag_global << std::endl; // = 0 
  // std::cout << "vflag_atom = "   << vflag_atom   << std::endl; // = 0
}
                    ...
                    double delx, double dely, double delz)
{
  ...
  if (vflag_either) {
    ...
    if (vflag_global) {
      if (newton_pair) {        
      // std::cout << "either here 2" << std::endl;
        virial[0] += v[0];
        virial[1] += v[1];
        virial[2] += v[2];
        virial[3] += v[3];
        ...
        if (i < nlocal) {
          // std::cout << "either here 3" << std::endl;
          virial[0] += 0.5*v[0];
          virial[1] += 0.5*v[1];
          virial[2] += 0.5*v[2];
          virial[3] += 0.5*v[3];
        ...
        if (j < nlocal) {
          // std::cout << "either here 4" << std::endl;
          virial[0] += 0.5*v[0];
          virial[1] += 0.5*v[1];
          virial[2] += 0.5*v[2];
          virial[3] += 0.5*v[3];
    ...
    if (vflag_atom) {
      if (newton_pair || i < nlocal) {
        // std::cout << "either here 5" << std::endl;
        vatom[i][0] += 0.5*v[0];
        vatom[i][1] += 0.5*v[1];
        vatom[i][2] += 0.5*v[2];
        vatom[i][3] += 0.5*v[3];
      ...
      if (newton_pair || j < nlocal) {
        // std::cout << "either here 6" << std::endl;
        vatom[j][0] += 0.5*v[0];
        vatom[j][1] += 0.5*v[1];
        vatom[j][2] += 0.5*v[2];
        vatom[j][3] += 0.5*v[3];
...
/* ----------------------------------------------------------------------
   ADD RAD COMPUTATION for ev_tally -> ev_tallyrad 
   controlled by !atom->rad_flag in pair_eam_rad.cpp
------------------------------------------------------------------------- */

void Pair::ev_tallyrad(int i, int j, int nlocal, int newton_pair,
                    double evdwl, double ecoul, double fpair,
                    double delx, double dely, double delz,
                    double fradpair, double delxrad, double delyrad, double delzrad)
{
  double evdwlhalf,ecoulhalf,epairhalf,v[6];
  double vrad[6];
  // std::cout << "fix here pair.cpp" << std::endl;
  ...
  if (vflag_either) {
    ...
    // follow the same expansion for [(a+ar)(b+br)(c+cr) - abc ]
    /*vrad[0] = (delx+delxrad)*(delx+delxrad)*(fpair+fradpair) - v[0];
    vrad[1] = (dely+delyrad)*(dely+delyrad)*(fpair+fradpair) - v[1];
    vrad[2] = (delz+delzrad)*(delz+delzrad)*(fpair+fradpair) - v[2];
    vrad[3] = (delx+delxrad)*(dely+delyrad)*(fpair+fradpair) - v[3];
    vrad[4] = (delx+delxrad)*(delz+delzrad)*(fpair+fradpair) - v[4];
    vrad[5] = (dely+delyrad)*(delz+delzrad)*(fpair+fradpair) - v[5];*/
    /* ------------------------------------------------------------- */
    // version 2 - Kaucher arithmetics
    vrad[0] = MathExtra::rad_TripleMult(delx,delxrad,delx,delxrad,fpair,fradpair);
    vrad[1] = MathExtra::rad_TripleMult(dely,delyrad,dely,delyrad,fpair,fradpair);
    vrad[2] = MathExtra::rad_TripleMult(delz,delzrad,delz,delzrad,fpair,fradpair);
    vrad[3] = MathExtra::rad_TripleMult(delx,delxrad,dely,delyrad,fpair,fradpair);
    vrad[4] = MathExtra::rad_TripleMult(delx,delxrad,delz,delzrad,fpair,fradpair);
    vrad[5] = MathExtra::rad_TripleMult(dely,delyrad,delz,delzrad,fpair,fradpair);

    // test - simple test to take out the uncertainty associated by fdotr
    // vrad[0] = 0.0;
    // vrad[1] = 0.0;
    // vrad[2] = 0.0;
    // vrad[3] = 0.0;
    ...
    if (vflag_global) {
      if (newton_pair) {
        ...
        if (i < nlocal) {
          ...
          // add rad
          virialrad[0] += 0.5*vrad[0];
          virialrad[1] += 0.5*vrad[1];
          virialrad[2] += 0.5*vrad[2];
          virialrad[3] += 0.5*vrad[3];
          virialrad[4] += 0.5*vrad[4];
          virialrad[5] += 0.5*vrad[5];
        }
        if (j < nlocal) {
          virial[0] += 0.5*v[0];
          virial[1] += 0.5*v[1];
    ...
    if (vflag_atom) {
      // std::cout << "caution pair.cpp" << std::endl;
      if (newton_pair || i < nlocal) {
        vatom[i][0] += 0.5*v[0];
        vatom[i][1] += 0.5*v[1];
        vatom[i][2] += 0.5*v[2];
...
void Pair::virial_fdotr_compute()
{
  ...
  if (neighbor->includegroup == 0) {
    ...
    for (int i = 0; i < nall; i++) {
      ...
      // if (atom->xrad_flag && atom->vrad_flag && atom->frad_flag)
      // {
      //   virialrad[0] += frad[i][0]*x[i][0] + f[i][0]*xrad[i][0] + frad[i][0]*xrad[i][0];
      //   virialrad[1] += frad[i][1]*x[i][1] + f[i][1]*xrad[i][1] + frad[i][1]*xrad[i][1];
      //   virialrad[2] += frad[i][2]*x[i][2] + f[i][2]*xrad[i][2] + frad[i][2]*xrad[i][2];
      //   virialrad[3] += frad[i][1]*x[i][0] + f[i][1]*xrad[i][0] + frad[i][1]*xrad[i][0];
      //   virialrad[4] += frad[i][2]*x[i][0] + f[i][2]*xrad[i][0] + frad[i][2]*xrad[i][0];
      //   virialrad[5] += frad[i][2]*x[i][1] + f[i][2]*xrad[i][1] + frad[i][2]*xrad[i][1];
      // }
    }
...
/* ----------------------------------------------------------------------
   update virial_ub with f(ub)dotr
------------------------------------------------------------------------- */
void Pair::virial_ub_fdotr_compute()
{
  double **x = atom->x;
  double **xub = atom->xub;
  double **fub = atom->fub;
  ...
  if (neighbor->includegroup == 0) {
    int nall = atom->nlocal + atom->nghost;
    for (int i = 0; i < nall; i++) {
      virial_ub[0] += fub[i][0]*x[i][0];
      virial_ub[1] += fub[i][1]*x[i][1];
      virial_ub[2] += fub[i][2]*x[i][2];
      virial_ub[3] += fub[i][1]*x[i][0];
      virial_ub[4] += fub[i][2]*x[i][0];
      virial_ub[5] += fub[i][2]*x[i][1];

      // ub version
      // virial_ub[0] += fub[i][0]*xub[i][0];
      // virial_ub[1] += fub[i][1]*xub[i][1];
      // virial_ub[2] += fub[i][2]*xub[i][2];
      // virial_ub[3] += fub[i][1]*xub[i][0];
      // virial_ub[4] += fub[i][2]*xub[i][0];
      // virial_ub[5] += fub[i][2]*xub[i][1];
    }
  ...
  // begin debug
  // for (int j = 0; j < 6; j++)
  //   std::cout << "virial_ub[" << j << "] = " << virial_ub[j] << std::endl;
  // end debug
}
/* ----------------------------------------------------------------------
   update virial_lb with f(lb)dotr
------------------------------------------------------------------------- */
void Pair::virial_lb_fdotr_compute()
{
  double **x = atom->x;
  double **xlb = atom->xlb;
  double **flb = atom->flb;
  ...
  if (neighbor->includegroup == 0) {
    int nall = atom->nlocal + atom->nghost;
    for (int i = 0; i < nall; i++) {
      virial_lb[0] += flb[i][0]*x[i][0];
      virial_lb[1] += flb[i][1]*x[i][1];
      virial_lb[2] += flb[i][2]*x[i][2];
      virial_lb[3] += flb[i][1]*x[i][0];
      virial_lb[4] += flb[i][2]*x[i][0];
      virial_lb[5] += flb[i][2]*x[i][1];

      // virial_lb[0] += flb[i][0]*xlb[i][0];
      // virial_lb[1] += flb[i][1]*xlb[i][1];
      // virial_lb[2] += flb[i][2]*xlb[i][2];
      // virial_lb[3] += flb[i][1]*xlb[i][0];
      // virial_lb[4] += flb[i][2]*xlb[i][0];
      // virial_lb[5] += flb[i][2]*xlb[i][1];
    }
...
/* ----------------------------------------------------------------------
   tallying UPPERBOUND VIRIAL
------------------------------------------------------------------------- */

void Pair::ev_tally_ub(int i, int j, int nlocal, int newton_pair,
                    double evdwl, double ecoul, double fpair,
                    double delx, double dely, double delz)
{
  double evdwlhalf,ecoulhalf,epairhalf,v_ub[6];
  double vrad_ub[6];
  // std::cout << "vflag_either = " << vflag_either << std::endl;
  ...
  if (vflag_either) {
    v_ub[0] = delx*delx*fpair;
    v_ub[1] = dely*dely*fpair;
    v_ub[2] = delz*delz*fpair;
    v_ub[3] = delx*dely*fpair;
    v_ub[4] = delx*delz*fpair;
    v_ub[5] = dely*delz*fpair;
    ...
    if (vflag_global) {
      if (newton_pair) {
        ...
        if (i < nlocal) {
          virial_ub[0] += 0.5*v_ub[0];
          virial_ub[1] += 0.5*v_ub[1];
          virial_ub[2] += 0.5*v_ub[2];
          virial_ub[3] += 0.5*v_ub[3];
          virial_ub[4] += 0.5*v_ub[4];
          virial_ub[5] += 0.5*v_ub[5];
          // std::cout << "either here" << std::endl;
        }
        if (j < nlocal) {
          virial_ub[0] += 0.5*v_ub[0];
    ...
    if (vflag_atom) {
      if (newton_pair || i < nlocal) {
        vatom[i][0] += 0.5*v_ub[0];
        vatom[i][1] += 0.5*v_ub[1];
        vatom[i][2] += 0.5*v_ub[2];
        vatom[i][3] += 0.5*v_ub[3];
        vatom[i][4] += 0.5*v_ub[4];
        vatom[i][5] += 0.5*v_ub[5];
      }
      if (newton_pair || j < nlocal) {
        vatom[j][0] += 0.5*v_ub[0];
        vatom[j][1] += 0.5*v_ub[1];
        vatom[j][2] += 0.5*v_ub[2];
        vatom[j][3] += 0.5*v_ub[3];
        vatom[j][4] += 0.5*v_ub[4];
        vatom[j][5] += 0.5*v_ub[5];
      }
    }
  }
}
...
/* ----------------------------------------------------------------------
   tallying LOWERBOUND VIRIAL
------------------------------------------------------------------------- */

void Pair::ev_tally_lb(int i, int j, int nlocal, int newton_pair,
                    double evdwl, double ecoul, double fpair,
                    double delx, double dely, double delz)
{
  double evdwlhalf,ecoulhalf,epairhalf,v_lb[6];
  double vrad_lb[6];
  ...
  if (vflag_either) {
    v_lb[0] = delx*delx*fpair;
    v_lb[1] = dely*dely*fpair;
    v_lb[2] = delz*delz*fpair;
    v_lb[3] = delx*dely*fpair;
    v_lb[4] = delx*delz*fpair;
    v_lb[5] = dely*delz*fpair;
    ...
    if (vflag_global) {
      if (newton_pair) {
        ...
        if (i < nlocal) {
          virial_lb[0] += 0.5*v_lb[0];
          virial_lb[1] += 0.5*v_lb[1];
          virial_lb[2] += 0.5*v_lb[2];
          virial_lb[3] += 0.5*v_lb[3];
          virial_lb[4] += 0.5*v_lb[4];
          virial_lb[5] += 0.5*v_lb[5];
          // std::cout << "or here 1" << std::endl;
        }
        if (j < nlocal) {
          virial_lb[0] += 0.5*v_lb[0];
          virial_lb[1] += 0.5*v_lb[1];
    ...
    if (vflag_atom) {
      if (newton_pair || i < nlocal) {
        vatom[i][0] += 0.5*v_lb[0];
        vatom[i][1] += 0.5*v_lb[1];
        vatom[i][2] += 0.5*v_lb[2];
        vatom[i][3] += 0.5*v_lb[3];
        vatom[i][4] += 0.5*v_lb[4];
        vatom[i][5] += 0.5*v_lb[5];
      }
      if (newton_pair || j < nlocal) {
        vatom[j][0] += 0.5*v_lb[0];
        vatom[j][1] += 0.5*v_lb[1];
        vatom[j][2] += 0.5*v_lb[2];
        vatom[j][3] += 0.5*v_lb[3];
        vatom[j][4] += 0.5*v_lb[4];
        vatom[j][5] += 0.5*v_lb[5];
      }
    }
  }
}
...
void Pair::update_virial_rad()
{
  for (int i = 0; i < 6; i++)
  {
    ...
    // end debug
    virialrad[i] = fmin( virial_ub[i] - virial[i], virial[i] - virial_lb[i]);
    // std::cout << "virialrad[i] = " << virialrad[i] << std::endl;
    // std::cout << "update_virial_rad()" << std::endl;
...
/* ----------------------------------------------------------------------
   MODIFIED VERSION for upperbound and lower computation
   setup for energy, virial computation
   see integrate::ev_set() for values of eflag (0-3) and vflag (0-6)
------------------------------------------------------------------------- */

void Pair::ev_setup_ub(int eflag, int vflag)
{
  int i,n;
...
/* ----------------------------------------------------------------------
   ...
   see integrate::ev_set() for values of eflag (0-3) and vflag (0-6)
------------------------------------------------------------------------- */
void Pair::ev_setup_lb(int eflag, int vflag)
{
  int i,n;
