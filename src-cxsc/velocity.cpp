...
void Velocity::create(double t_desired, int seed)
{
  ...
  // begin debug
  // status: only run at equilbriation
  // std::cout << "running velocity.cpp ... " << std::endl;
  // end debug
  ...
  if (loop_flag == ALL) {
    ...
    for (i = 1; i <= natoms; i++) {
      ...
      if (m >= 0 && m < nlocal) {
        if (mask[m] & groupbit) {
          ...
          if (dimension == 3 && atom->vrad_flag) vrad[m][2] = 0.0;
        }
      }
    }
    ...
    for (i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (dimension == 3 && atom->vrad_flag) vrad[i][2] = 0.0;
      }
    }
...
void Velocity::set(int narg, char **arg)
{
  ...
  if (varflag == CONSTANT) {
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        if (sum_flag == 0) {
          if (xstyle) {v[i][0] = vx; vub[i][0] = vx; vlb[i][0] = vx; vrad[i][0] = 0.0; }
          if (ystyle) {v[i][1] = vy; vub[i][1] = vy; vlb[i][1] = vy; vrad[i][1] = 0.0; }
          if (zstyle) {v[i][2] = vz; vub[i][2] = vz; vlb[i][2] = vz; vrad[i][2] = 0.0; }
          if (atom->vrad_flag) {
            vrad[i][0] = 0.0;
            vrad[i][1] = 0.0;
            vrad[i][2] = 0.0;
  ...
  // set velocities via variables
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        if (sum_flag == 0) {
          if (xstyle == ATOM) {v[i][0] = vfield[i][0]; vub[i][0] = vfield[i][0]; vlb[i][0] = vfield[i][0]; vrad[i][0] = 0.0;}
          else if (xstyle) {v[i][0] = vx; vub[i][0] = vx; vlb[i][0] = vx; vrad[i][0] = 0.0;}
          if (ystyle == ATOM) {v[i][1] = vfield[i][1]; vub[i][1] = vfield[i][1]; vlb[i][1] = vfield[i][1]; vrad[i][1] = 0.0;}
          else if (ystyle) {v[i][1] = vy; vub[i][1] = vy; vlb[i][1] = vy; vrad[i][1] = 0.0;}
          if (zstyle == ATOM) {v[i][2] = vfield[i][2]; vub[i][2] = vfield[i][2]; vlb[i][2] = vfield[i][2]; vrad[i][2] = 0.0;}
          else if (zstyle) {v[i][2] = vz; vub[i][2] = vz; vlb[i][2] = vz; vrad[i][2] = 0.0;}
        } else {
          if (xstyle == ATOM) {v[i][0] += vfield[i][0]; vub[i][0] += vfield[i][0]; vlb[i][0] += vfield[i][0]; vrad[i][0] = 0.0;}
          else if (xstyle) {v[i][0] += vx; vub[i][0] += vx; vlb[i][0] += vx; vrad[i][0] = 0.0;}
          if (ystyle == ATOM) {v[i][1] += vfield[i][1]; vub[i][1] += vfield[i][1]; vlb[i][1] += vfield[i][1]; vrad[i][1] = 0.0;}
...
void Velocity::ramp(int narg, char **arg)
{
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      if (sum_flag) {v[i][v_dim] += vramp; vub[i][v_dim] += vramp; vlb[i][v_dim] += vramp; }
      else {v[i][v_dim] = vramp; vub[i][v_dim] = vramp; vlb[i][v_dim] = vramp; }
    }
}
...
void Velocity::zero_rotation()
{
  ...
  for (i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      vub[i][0] -= omega[1]*dz - omega[2]*dy;
      vub[i][1] -= omega[2]*dx - omega[0]*dz;
      vub[i][2] -= omega[0]*dy - omega[1]*dx;
      vlb[i][0] -= omega[1]*dz - omega[2]*dy;
      vlb[i][1] -= omega[2]*dx - omega[0]*dz;
      vlb[i][2] -= omega[0]*dy - omega[1]*dx;
      if (atom->vrad_flag)
      {
        vrad[i][0] = 0;
        vrad[i][1] = 0;
