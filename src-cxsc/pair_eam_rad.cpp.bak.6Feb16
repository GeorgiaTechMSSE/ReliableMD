...
void PairEAMRad::compute(int eflag, int vflag)
{
  ...
  compute_ublb(eflag,vflag); // combine method
...
void PairEAMRad::read_file(char *filename)
{
  ...
  if (me == 0 && atom->frad_flag)
  {
    ...
    // print out the error generating parameters
...
void PairEAMRad::unpack_comm(int n, int first, double *buf)
{
  ...
  if (pack_flag == 1) {
    ...
    for (i = first; i < last; i++) fp_ub[i] = buf[m++];
  } else if (pack_flag == 3) {
    for (i = first; i < last; i++) fp_lb[i] = buf[m++];
  }
}
...
int PairEAMRad::pack_reverse_comm(int n, int first, double *buf)
{
  ...
  if (pack_flag == 1) {
    ...
    for (i = first; i < last; i++) buf[m++] = rho_ub[i];
  }
  else if (pack_flag == 3) {
    for (i = first; i < last; i++) buf[m++] = rho_lb[i];
  }
  return 1;
}
...
------------------------------------------------------------------------- */
// par PairEAMRad::compute(), this function computes the upper/lower bounds of the forces based on the IDETC paper
void PairEAMRad::compute_ublb(int eflag, int vflag)
{
  int i,j,ii,jj,m,inum,jnum,itype,jtype;
  ...
  evdwl = 0.0;
  if (eflag || vflag) {
    ev_setup_ub(eflag,vflag); 
    ev_setup_lb(eflag,vflag);
  }
  else evflag = vflag_fdotr = eflag_global = eflag_atom = 0;
  ...
  if (atom->nmax > nmax) {
    ...
    memory->create(rho_lb,nmax,"pair:rho_ub");
  ...
  // ****************************************************************************** //
  // short-goal: compute rho[i] - density at each atom
  ...
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    xtmp_ub = xub[i][0]; // upperbound computation
    ytmp_ub = xub[i][1]; // upperbound computation
    ztmp_ub = xub[i][2]; // upperbound computation
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      delx_ub = xtmp_ub - xub[j][0]; // upperbound computation
      dely_ub = ytmp_ub - xub[j][1]; // upperbound computation
      delz_ub = ztmp_ub - xub[j][2]; // upperbound computation

      rsq_ub = delx_ub*delx_ub + dely_ub*dely_ub + delz_ub*delz_ub;

      if (rsq_ub < cutforcesq) {
        jtype = type[j];
        p = sqrt(rsq_ub)*rdr + 1.0;
        m = static_cast<int> (p);
        m = MIN(m,nr-1);
        p -= m;
        p = MIN(p,1.0);
        coeff = rhor_spline[type2rhor[jtype][itype]][m];
        rho_ub[i] += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
        if (newton_pair || j < nlocal) {
          coeff = rhor_spline[type2rhor[itype][jtype]][m];
          // compute rho_ub[i] 
          rho_ub[j] += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
        }
      }
    }
  }
  // add pack_flag = 2 or 3
  pack_flag = 2;
  if (newton_pair) comm->reverse_comm_pair(this);
  ...
  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    xtmp_lb = xlb[i][0]; // lowerbound computation
    ytmp_lb = xlb[i][1]; // lowerbound computation
    ztmp_lb = xlb[i][2]; // lowerbound computation
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      delx_lb = xtmp_lb - xlb[j][0]; // upperbound computation
      dely_lb = ytmp_lb - xlb[j][1]; // upperbound computation
      delz_lb = ztmp_lb - xlb[j][2]; // upperbound computation

      rsq_lb = delx_lb*delx_lb + dely_lb*dely_lb + delz_lb*delz_lb;

      if (rsq_lb < cutforcesq) {
        jtype = type[j];
        p = sqrt(rsq_lb)*rdr + 1.0;
        m = static_cast<int> (p);
        m = MIN(m,nr-1);
        p -= m;
        p = MIN(p,1.0);
        coeff = rhor_spline[type2rhor[jtype][itype]][m];
        rho_lb[i] += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
        if (newton_pair || j < nlocal) {
          coeff = rhor_spline[type2rhor[itype][jtype]][m];
          // compute rho_lb[i] 
          rho_lb[j] += ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
        }
      }
    }
  }
  ...
  // short-goal: compute fp[i] - derivative of embedding energy at each atom
  // communicate and sum densities
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // compute fp_ub[i] and fp_lb[i]
    fp_ub[i] = (coeff[0]*p + coeff[1])*p + coeff[2];
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // compute fp_ub[i] and fp_lb[i]
    fp_lb[i] = (coeff[0]*p + coeff[1])*p + coeff[2];
  }
