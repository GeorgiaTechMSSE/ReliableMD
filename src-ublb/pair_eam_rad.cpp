...
void PairEAMRad::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    // frad[i][0] = fmax( fub[i][0] - f[i][0], f[i][0] - flb[i][0]);
    // frad[i][1] = fmax( fub[i][1] - f[i][1], f[i][1] - flb[i][1]);
    // frad[i][2] = fmax( fub[i][2] - f[i][2], f[i][2] - flb[i][2]);
    ...
    for (int k = 0; k < 3; k++) {
      if (flb[i][k] < fub[i][k]) frad[i][k] = fmin(fub[i][k] - f[i][k] , f[i][k] - flb[i][k]);
      else frad[i][k] = fmax(fub[i][k] - f[i][k] , f[i][k] - flb[i][k]);
    }
    // outer radius computation
    // for (int k = 0; k < 3; k++) {
    //   if (flb[i][k] < fub[i][k]) frad[i][k] = fmax(fub[i][k] - f[i][k] , f[i][k] - flb[i][k]);
    //   else frad[i][k] = fmin(fub[i][k] - f[i][k] , f[i][k] - flb[i][k]);
    // }
  }
...
void PairEAMRad::compute_ub(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    p = MIN(p,1.0);
    coeff = frho_spline[type2frho[type[i]]][m];
    fp[i] = (coeff[0]*p + coeff[1])*p + coeff[2] + type2err_dev(Frad_a, Frad_b, rho_0, r); // upperbound computation
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutforcesq) {
        ...
        coeff = rhor_spline[type2rhor[itype][jtype]][m];
        rhoip = (coeff[0]*p + coeff[1])*p + coeff[2] + type1err_dev(rhorad_a, rhorad_b, r); // upperbound computation
        // rhoip = (coeff[0]*p + coeff[1])*p + coeff[2]; // test
        coeff = rhor_spline[type2rhor[jtype][itype]][m];
        rhojp = (coeff[0]*p + coeff[1])*p + coeff[2] + type1err_dev(rhorad_a, rhorad_b, r); // upperbound computation
        rhojp = (coeff[0]*p + coeff[1])*p + coeff[2]; // test
        coeff = z2r_spline[type2z2r[itype][jtype]][m];
        z2p = (coeff[0]*p + coeff[1])*p + coeff[2] + type1err_dev(zrad_a, zrad_b, r); // upperbound computation
        // z2p = (coeff[0]*p + coeff[1])*p + coeff[2]; // test
        z2 = ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
...
void PairEAMRad::compute_lb(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    p = MIN(p,1.0);
    coeff = frho_spline[type2frho[type[i]]][m];
    fp[i] = (coeff[0]*p + coeff[1])*p + coeff[2] - type2err_dev(Frad_a, Frad_b, rho_0, r); // lowerbound computation
    // if (eflag) {
    //   phi = ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
    //   if (rho[i] > rhomax) phi += fp[i] * (rho[i]-rhomax);
    //   if (eflag_global) eng_vdwl += phi;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutforcesq) {
        ...
        coeff = rhor_spline[type2rhor[itype][jtype]][m];
        rhoip = (coeff[0]*p + coeff[1])*p + coeff[2] - type1err_dev(rhorad_a, rhorad_b, r); // lowerbound computation
        coeff = rhor_spline[type2rhor[jtype][itype]][m];
        rhojp = (coeff[0]*p + coeff[1])*p + coeff[2] - type1err_dev(rhorad_a, rhorad_b, r); // lowerbound computation
        coeff = z2r_spline[type2z2r[itype][jtype]][m];
        z2p = (coeff[0]*p + coeff[1])*p + coeff[2] - type1err_dev(zrad_a, zrad_b, r); // lowerbound computation
        z2 = ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];
