...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,
    XRAD,YRAD,ZRAD,
    VXRAD,VYRAD,VZRAD,
    CHARGE,MASS,SHAPE,LENGTH,TRI,
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    /* ------------------------------------------------- */
    // add rad
    else if (keyword == XRAD) atom->xrad[i][0] = dvalue;
    else if (keyword == YRAD) atom->xrad[i][1] = dvalue;
    else if (keyword == ZRAD) atom->xrad[i][2] = dvalue;
    else if (keyword == VXRAD) atom->vrad[i][0] = dvalue;
    else if (keyword == VYRAD) atom->vrad[i][1] = dvalue;
    else if (keyword == VZRAD) atom->vrad[i][2] = dvalue;
    /* ------------------------------------------------- */
