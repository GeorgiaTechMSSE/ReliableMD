...
{
  double inf,sup;
  interval x_intval (x-xrad,x+xrad);
  interval y_intval (y-yrad,y+yrad);
  interval z_intval (z-xrad,z+zrad);
  interval TripleProduct = (x_intval*y_intval) * z_intval;
  return ( 0.5 * (TripleProduct.Sup() - TripleProduct.Inf()) );
}
