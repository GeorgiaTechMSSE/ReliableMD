...
void Velocity::set(int narg, char **arg)
{
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        if (sum_flag == 0) {
          ...
          if (xstyle) {v[i][0] += vx; vrad[i][0] = 0.0;}
          if (ystyle) {v[i][1] += vy; vrad[i][1] = 0.0;}
          if (zstyle) {v[i][2] += vz; vrad[i][2] = 0.0;}
        }
      }
    }
  ...
  // set velocities via variables
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        if (sum_flag == 0) {
          if (xstyle == ATOM) {v[i][0] = vfield[i][0]; vrad[i][0] = 0.0;}
          else if (xstyle) {v[i][0] = vx; vrad[i][0] = 0.0;}
          if (ystyle == ATOM) {v[i][1] = vfield[i][1]; vrad[i][1] = 0.0;}
          else if (ystyle) {v[i][1] = vy; vrad[i][1] = 0.0;}
          if (zstyle == ATOM) {v[i][2] = vfield[i][2]; vrad[i][2] = 0.0;}
          else if (zstyle) {v[i][2] = vz; vrad[i][2] = 0.0;}
        } else {
          if (xstyle == ATOM) {v[i][0] += vfield[i][0]; vrad[i][0] = 0.0;}
          else if (xstyle) {v[i][0] += vx; vrad[i][0] = 0.0;}
          if (ystyle == ATOM) {v[i][1] += vfield[i][1]; vrad[i][1] = 0.0;}
