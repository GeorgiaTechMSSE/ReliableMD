    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (atom->vrad_flag)
        {
          ...
          // vrad[i][1] *= factor[1];
          // vrad[i][2] *= factor[2];
          vrad[i][0] = 0.001 * abs(v[i][0]);
          vrad[i][1] = 0.001 * abs(v[i][1]);
          vrad[i][2] = 0.001 * abs(v[i][2]);
        }
        if (pstyle == TRICLINIC) {
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (atom->vrad_flag)
        {
          ...
          // vrad[i][1] *= factor[1];
          // vrad[i][2] *= factor[2];
          vrad[i][0] = 0.01 * abs(v[i][0]);
          vrad[i][1] = 0.01 * abs(v[i][1]);
          vrad[i][2] = 0.01 * abs(v[i][2]);
        }
  ...
  if (rmass) {
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        {
          // for (int k = 0; k<3; k++) {
          //   if (vrad[i][k] == NULL) std::cout << "Warning: vrad[i][" << k << "] = NULL" << std::endl;
          //   if (vrad[i][k] == NULL) std::cout << "atom number (i) = " << i << std::endl;
          // }
          // std::cout << "dtfm = " << dtfm << std::endl;
