...
void Pair::update_virial_rad()
{
  for (int i = 0; i < 6; i++)
  {
    ...
    if (virial_lb[i] < virial_ub[i])
      virialrad[i] = fmin( virial_ub[i] - virial[i], virial[i] - virial_lb[i]);
    else virialrad[i] = fmax( virial_ub[i] - virial[i], virial[i] - virial_lb[i]);

    // outer radius computation
    // if (virial_lb[i] < virial_ub[i])
    //   virialrad[i] = fmax( virial_ub[i] - virial[i], virial[i] - virial_lb[i]);
    // else virialrad[i] = fmin( virial_ub[i] - virial[i], virial[i] - virial_lb[i]);
  }
}
/* ----------------------------------------------------------------------
