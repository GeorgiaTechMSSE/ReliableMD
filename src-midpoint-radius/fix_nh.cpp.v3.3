    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (pstyle == TRICLINIC) {
          ...
          if (atom->vrad_flag)
          {
            vrad[i][0] += -dthalf*(vrad[i][1]*omega_dot[5] + vrad[i][2]*omega_dot[4]);
            vrad[i][1] += -dthalf*vrad[i][2]*omega_dot[3];
            vub[i][0] += -dthalf*(vub[i][1]*omega_dot[5] + vub[i][2]*omega_dot[4]);
            vub[i][1] += -dthalf*vub[i][2]*omega_dot[3];
            vlb[i][0] += -dthalf*(vlb[i][1]*omega_dot[5] + vlb[i][2]*omega_dot[4]);
            vlb[i][1] += -dthalf*vlb[i][2]*omega_dot[3];
          }
        }
        v[i][0] *= factor[0];
        v[i][1] *= factor[1];
  ...
  if (rmass) {
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        v[i][2] += dtfm*f[i][2];
        /* ---------------------------------------------------- */
        // update vlb and vub based on flb and fub - version 3
        if (atom->xrad_flag && atom->vrad_flag && atom->frad_flag) {
          vlb[i][0] = v[i][0] + dtfm*flb[i][0];
          vlb[i][1] = v[i][1] + dtfm*flb[i][1];
          vlb[i][2] = v[i][2] + dtfm*flb[i][2];
          vub[i][0] = v[i][0] + dtfm*fub[i][0];
          vub[i][1] = v[i][1] + dtfm*fub[i][1];
          vub[i][2] = v[i][2] + dtfm*fub[i][2];
        }
        // update vrad - version 1 and version 2
        if (atom->vrad_flag && atom->frad_flag) {
          vrad[i][0] += dtfm*frad[i][0];
          vrad[i][1] += dtfm*frad[i][1];
          vrad[i][2] += dtfm*frad[i][2];
        }
        /* ---------------------------------------------------- */
      }
    }
...
void FixNH::nve_x()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      x[i][2] += dtv * v[i][2];
      /* ---------------------------------------------------- */
      // update xlb and xub based on vlb and vub - version 3
      if (atom->xrad_flag && atom->vrad_flag && atom->frad_flag) {
        xlb[i][0] = x[i][0] + dtv * vlb[i][0];
        xlb[i][1] = x[i][1] + dtv * vlb[i][1];
        xlb[i][2] = x[i][2] + dtv * vlb[i][2];
        xub[i][0] = x[i][0] + dtv * vub[i][0];
        xub[i][1] = x[i][1] + dtv * vub[i][1];
        xub[i][2] = x[i][2] + dtv * vub[i][2];
      }
      // update xrad - version 1 and version 2
      if (atom->xrad_flag && atom->vrad_flag) {
        xrad[i][0] += dtv * vrad[i][0];
        xrad[i][1] += dtv * vrad[i][1];
        xrad[i][2] += dtv * vrad[i][2];
      }
      /* ---------------------------------------------------- */
    }
  }
